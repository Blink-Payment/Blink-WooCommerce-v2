...
function checkOrderPayment($order_id)
{
    ...
    if (!$order->needs_payment()) {
        wc_add_notice(
            "Something Wrong! Please initate the payment from checkout page",
            "error"
        );
    ...
    if (isset($blinkPay) && "" !== $blinkPay) {
        ...
        if (!empty($method) && in_array($method, $gateWay->paymentMethods)) {
            ...
            if (isset($gateWay->paymentIntent["payment_intent"])) {
                ...
                $html .=
                    '<section class="blink-api-section">
                            <div class="blink-api-form-stracture">
                                <h2 class="heading-text">Pay with ' .
                    $string .
                    '</h2>
                ...
                $html .=
                    '<input type="hidden" name="transaction_unique" value="' .
                    $gateWay->paymentIntent["transaction_unique"] .
                    '">
                                                <input type="hidden" name="amount" value="' .
                    $gateWay->paymentIntent["amount"] .
                    '">
                                                <input type="hidden" name="intent_id" value="' .
                    $gateWay->paymentIntent["id"] .
                    '">
                                                <input type="hidden" name="intent" value="' .
                    $gateWay->paymentIntent["payment_intent"] .
                    '">
...
function wc_blink_missing_notice()
{
    ...
    echo '<div class="error"><p><strong>' .
        sprintf(
            esc_html__(
                "Blink requires WooCommerce to be installed and active. You can download %s here.",
                "blink"
            ),
...
function blink_init_gateway_class()
{
    ...
    add_filter(
        "plugin_action_links",
        "add_wc_blink_payment_action_plugin",
        10,
        5
    );
