...
class WC_Blink_Gateway extends WC_Payment_Gateway
{
    public function __construct()
    {
        ...
        $this->id = str_replace(
            " ",
            "",
            strtolower($this->configs["method_title"])
        );
        ...
        add_action("woocommerce_thankyou_blink", [
            $this,
            ...
            if (!empty($payment_types)) {
                if (empty($this->paymentMethods)) {
                    if (!$adminnotice->has_notice("no-payment-type-selected")) {
                        $adminnotice->add_custom_notice(
                            "no-payment-type-selected",
                            "<div>Please select the Payment Methods and save the configuration!</div>"
                        );
                ...
                if (!$adminnotice->has_notice("no-payment-types")) {
                    $adminnotice->add_custom_notice(
                        "no-payment-types",
                        "<div>There is no Payment Types Available</div>"
                    );
                    $adminnotice->output_custom_notices();
                }
            }
    ...
    public function init_form_fields($payment_types = [])
    {
        $fields = [
            ...
            "description" => [
                ...
                "default" =>
                    "Pay with your credit card or direct debit at your convenience.",
            ],
            "testmode" => [
                "title" => "Test mode",
    ...
    public function payment_fields()
    {
        if (is_array($this->paymentMethods) && empty($this->paymentMethods)) {
            echo wpautop(
                wp_kses_post("Unable to process any payment at this moment!")
            );
        } else {
            if ($this->description) {
...
<?php }
    ...
    public function get_order_data($order)
    {
        return $order_data = [
            ...
            "order_date" => date(
                "Y-m-d H:i:s",
                strtotime(get_post($order->get_id())->post_date)
            ),
            "shipping_total" => $order->get_total_shipping(),
            "shipping_tax_total" => wc_format_decimal(
                $order->get_shipping_tax(),
                2
            ),
            "tax_total" => wc_format_decimal($order->get_total_tax(), 2),
            "cart_discount" =>
                defined("WC_VERSION") && WC_VERSION >= 2.3
                    ? wc_format_decimal($order->get_total_discount(), 2)
                    : wc_format_decimal($order->get_cart_discount(), 2),
            "order_discount" =>
                defined("WC_VERSION") && WC_VERSION >= 2.3
                    ? wc_format_decimal($order->get_total_discount(), 2)
                    : wc_format_decimal($order->get_order_discount(), 2),
            "discount_total" => wc_format_decimal(
    ...
    public function get_payment_information($order_id)
    {
        $order = wc_get_order($order_id);
        return json_encode([
            "payer_info" => $this->get_customer_data($order),
            "order_info" => $this->get_order_data($order),
        ]);
    }
    ...
    public function processCreditCard($order_id, $request)
    {
        ...
        if (isset($request["remote_address"])) {
            ...
            $requestData["device_capabilities"] =
                $request["device_capabilities"];
            $requestData["device_accept_language"] =
                $request["device_accept_language"];
            $requestData["device_screen_resolution"] =
                $request["device_screen_resolution"];
    ...
    public function change_status(
        ...
        if (
            "captured" === strtolower($status) ||
            "success" === strtolower($status) ||
            "accept" === strtolower($status)
        ) {
            ...
            "pending submission" === strtolower($status)
        ) {
        ...
        if ($transaction_id) {
            ...
            if (!$order_id) {
                ...
                FROM " .
                        $wpdb->postmeta .
                        "
                WHERE (`meta_key` = '_transaction_id' AND `meta_value` = '" .
                        $transaction_id .
                        "') OR (`meta_key` = 'blink_res' AND `meta_value` = '" .
                        $transaction_id .
                        "')"
                );
            }
